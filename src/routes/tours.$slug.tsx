...
import { getStoredTours, getStoredSettings } from "@/lib/storage";
...
function getRegionsFromTitle(title: string): string[] {
  ...
  if (t.includes("alapuzha") || t.includes("alleppey") || t.includes("house boat"))
    foundRegions.push("alleppey");
...
function TourDetail() {
  ...
  const generateDynamicItinerary = () => {
    return Array.from({ length: tour.durationInDays }).map((_, idx) => {
      ...
      const regionName =
        regionForDay === "default"
          ? "Local"
          : regionForDay.charAt(0).toUpperCase() + regionForDay.slice(1);
      return {
        day: idx + 1,
  ...
  const itinerary =
    tour.itinerary &&
    tour.itinerary[0] &&
    tour.itinerary[0].title?.toLowerCase().includes("best spots")
    ...
    <div className="bg-background text-foreground min-h-screen selection:bg-gold selection:text-primary-foreground">
          ...
          <div className="mx-auto max-w-3xl px-6">
            ...
            >
              <div className="flex flex-col items-center">
                <span className="font-mono text-[10px] uppercase tracking-widest text-white/50 mb-1">
                  Expedition Span
                </span>
          ...
          >
            ...
            <motion.div variants={itemVariants} className="mt-16 grid gap-8 sm:grid-cols-2">
              <div className="group rounded-2xl border border-border/40 bg-card/30 p-8 transition-all hover:bg-card/50 hover:border-gold/30">
                ...
                <h3 className="font-display text-xl tracking-tight mb-2 uppercase">
                  Signature Transport
                </h3>
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Travel in our signature high-fidelity coaches featuring custom lighting, premium
                  sound, and ergonomic reclining seats.
                </p>
              </div>
              <div className="group rounded-2xl border border-border/40 bg-card/30 p-8 transition-all hover:bg-card/50 hover:border-gold/30">
                <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-xl bg-gold/10 text-gold group-hover:bg-gold group-hover:text-primary-foreground transition-colors">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Hand-picked resorts and villas that maintain our standards of safety, hygiene, and
                  aesthetic appeal.
                </p>
              </div>
            </motion.div>
              ...
              <ul className="space-y-6">
                {tour.highlights.map((h, i) => (
                  <li key={i} className="flex items-center gap-4 group/item">
                    ...
                    <span className="text-xs font-bold tracking-[0.15em] uppercase text-foreground/80 group-hover/item:text-gold transition-colors">
                      {h}
                    </span>
                  </li>
                ))}
          ...
          <div className="relative space-y-12">
            ...
            {itinerary.map((day, idx) => (
              ...
              >
                ...
                <div
                  className={`md:flex flex-col ${idx % 2 === 0 ? "md:items-end md:text-right text-left pl-14 md:pl-0" : "md:col-start-2 text-left pl-14 md:pl-0"}`}
                >
                  <div className="flex items-center gap-4 mb-3 md:contents">
                    <span className="font-mono text-[10px] uppercase tracking-[0.4em] text-gold/60">
                      Day 0{day.day}
                    </span>
                    <div className="h-px bg-gold/20 flex-1 md:hidden" />
                  </div>
                  <h3 className="font-display text-3xl md:text-5xl tracking-tighter text-white uppercase mb-6">
                    {day.title}
                  </h3>
                </div>

                <div
                  className={`pl-14 md:pl-0 ${idx % 2 === 0 ? "md:col-start-2" : "md:text-right md:pr-10 md:flex md:flex-col md:items-end"}`}
                >
                  <ul className="space-y-4">
                    {day.activities.map((act, i) => (
                      <li
                        key={i}
                        className="group/act flex items-start gap-4 text-foreground/70 transition-colors hover:text-white"
                      >
                        <div className="mt-1 flex-shrink-0 opacity-20 transition-opacity group-hover/act:opacity-100">
                          <ArrowRight size={14} className="text-gold" />
                        </div>
                        <span className="text-base font-light tracking-wide leading-snug">
                          {act}
                        </span>
                      </li>
                    ))}
            ...
            >
              ...
              <ul className="grid gap-6 sm:grid-cols-1">
                {tour.exclusions.map((item, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-4 text-sm font-light tracking-[0.1em] uppercase text-white/40 italic"
                  >
        ...
        <div className="relative mx-auto max-w-4xl text-center">
          ...
          >
            ...
            <h2 className="font-display text-4xl sm:text-6xl md:text-8xl tracking-tight leading-[0.8] text-white uppercase mb-12">
              BEGIN YOUR <br />
              <span className="gradient-gold-text italic font-light lowercase">
                Emirates odyssey
              </span>
            </h2>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              ...
              <a
                ...
                className="text-white/60 hover:text-white transition-colors uppercase font-mono text-[10px] tracking-[0.4em] border-b border-white/10 pb-1"
              >
                Consult with an expert ({settings.phone1})
              </a>
            </div>
          </motion.div>
        </div>
      ...
      <div className="bg-background py-10 border-t border-white/5">
        <div className="mx-auto max-w-7xl px-6 lg:px-10 flex justify-between items-center">
          <Link
            to="/tours"
            className="flex items-center gap-3 text-gold hover:text-white transition-colors font-mono text-[10px] uppercase tracking-[0.3em] group"
          >
