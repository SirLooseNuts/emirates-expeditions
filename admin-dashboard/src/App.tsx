      ...
      itinerary: [
        {
          ...
          activities: ["Scenic sightseeing", "Photography"],
        },
      ],
      ...
      <div className="toast-container">
        {toasts.map((toast) => (
          <div
            ...
            className="toast"
            style={{
              borderLeftColor: toast.type === "error" ? "var(--destructive)" : "var(--gold)",
            }}
          >
            {toast.text}
          </div>
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Configure and release trip itineraries ({tours.length} packages loaded)
                      </p>
                    </div>
                    <button
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    {tours.map((tour) => (
                      <div
                        key={tour.slug}
                        className="relative flex flex-col justify-between border border-border bg-background p-5 rounded-sm transition-all hover:border-gold/50"
                      >
                        <div>
                          ...
                          <p className="font-mono text-xs text-gold/70 mt-1">
                            {tour.duration} · {tour.price}
                          </p>
                          <p className="text-xs text-muted-foreground mt-2 line-clamp-2">
                            {tour.blurb || "No description provided."}
                          </p>
                          ...
                          <div className="mt-4 flex flex-wrap gap-1.5">
                            {tour.highlights.slice(0, 3).map((h, i) => (
                              <span
                                key={i}
                                className="text-[9px] bg-white/5 border border-white/10 px-2 py-0.5 rounded-sm text-white/70"
                              >
                                {h}
                              </span>
                            ))}
              ...
              {activeTab === "gallery" && (
                <motion.div
                  ...
                  transition={{ duration: 0.3 }}
                >
                  <h2 className="font-display text-2xl tracking-wider uppercase text-white mb-2">
                    Gallery Manager
                  </h2>
                  <p className="text-xs text-muted-foreground mb-8">
                    Manage the media feed displayed on the homepage and gallery catalogue
                  </p>

                  <form
                    onSubmit={handleAddPhoto}
                    className="border border-border bg-background p-6 rounded-sm mb-8 flex flex-col gap-4"
                  >
                    ...
                    <div className="grid gap-4 sm:grid-cols-2">
                      <div>
                        <label className="font-mono text-[9px] uppercase tracking-widest text-white/50 block mb-1">
                          Image URL
                        </label>
                        <input
                          type="text"
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {photos.slice(0, 24).map((p, idx) => (
                      <div
                        key={idx}
                        className="relative aspect-square overflow-hidden rounded-sm border border-border bg-background group"
                      >
                        ...
                        <div className="absolute inset-0 bg-background/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-3 z-10">
                          ...
                          <div className="flex flex-col gap-2">
                            <p className="text-[10px] text-white/90 line-clamp-2 uppercase tracking-wide font-mono leading-tight">
                              {p.alt}
                            </p>
                            <button
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Approve client responses or manually draft custom reviews
                      </p>
                    </div>
                    <button
                  ...
                  <div className="flex flex-col gap-4">
                    {reviews.map((rev) => (
                      <div
                        key={rev.id}
                        className="border p-5 rounded-sm flex flex-col md:flex-row md:items-center justify-between gap-4 bg-background/40"
                      >
                        <div className="flex-1 font-mono">
                          ...
                          <p className="text-[10px] uppercase tracking-wider text-muted-foreground mt-3">
                            — {rev.author} · <span className="text-gold">{rev.location}</span> ·{" "}
                            {rev.tripType}
                          </p>
                        </div>
              ...
              {activeTab === "settings" && (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mb-8">
                    Edit contact parameters displayed on headers, footers, CTAs, and links
                  </p>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 overflow-y-auto">
            ...
            >
              ...
              <form onSubmit={handleSaveTour} className="flex flex-col gap-4 text-xs">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setEditingTour({
                          ...editingTour,
                          durationInDays: parseInt(e.target.value) || 1,
                        })
                      }
                      className="min-h-[40px] w-full rounded-sm border border-border bg-background px-3 py-2 text-xs text-white focus:border-gold focus:outline-none"
                    />
                ...
                <div>
                  <label className="font-mono text-[9px] uppercase tracking-widest text-white/50 block mb-1">
                    Full Detailed Itinerary Description
                  </label>
                  <textarea
                    rows={4}
                    ...
                    onChange={(e) =>
                      setEditingTour({ ...editingTour, fullDescription: e.target.value })
                    }
